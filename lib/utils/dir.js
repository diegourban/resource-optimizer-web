  ...
  return new Promise(function(resolve, reject) {
    ...
    while (stack.length) {
      ...
      if (currentStat.isDirectory()) {
        ...
        let extension = extensionutils.extensionToContentType(path.extname(currentExtractedPath));
        ...
        if(extension === "invalid") {
          ...
          const requestInfo = {
            ...
            body: fs.createReadStream(currentExtractedPath)
          }

          // envia para a API otimizar e guarda a promise
          let fetchCall = fetchutils.fetchAPI(requestInfo, currentOptimizedPath);
    ...
    .catch(err => {
      logger.error("Ocorreu um erro inesperado " + err);
      reject(err);
    });
  });
}
